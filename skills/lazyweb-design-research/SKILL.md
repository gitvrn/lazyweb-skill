...
```bash
IDEMPOTENCY_KEY="${REPORT_DIR#.lazyweb/}"   # stable per-report key (e.g. design-research/{topic}-{date}); send the SAME value every attempt so retries dedupe to one link
