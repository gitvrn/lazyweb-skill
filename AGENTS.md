...
These principles live in each skill's report contract. When editing a skill, keep its local component rules internally consistent. For `lazyweb-design-research`, keep the report v3 side-by-side compare, option deck, inspo map, and annotated pattern components in sync with this file; for the other skills, keep their existing shared evidence components in sync.
